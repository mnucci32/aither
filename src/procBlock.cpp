...
void procBlock::Prolongation(
  ...
  const auto coarseNodes = this->CellToNode();
  ...
  for (auto kk = fine.StartK(); kk < fine.EndK(); ++kk) {
    for (auto jj = fine.StartJ(); jj < fine.EndJ(); ++jj) {
      for (auto ii = fine.StartI(); ii < fine.EndI(); ++ii) {
        ...
        const auto fc = fine.center_(ii, jj, kk);
        // nodal coordinates of bounding coarse cell
        const auto c0 = nodes_.Coords(ci.X()    , ci.Y()    , ci.Z());
        const auto c1 = nodes_.Coords(ci.X() + 1, ci.Y()    , ci.Z());
        const auto c2 = nodes_.Coords(ci.X()    , ci.Y() + 1, ci.Z());
        const auto c3 = nodes_.Coords(ci.X() + 1, ci.Y() + 1, ci.Z());
        const auto c4 = nodes_.Coords(ci.X()    , ci.Y()    , ci.Z() + 1);
        const auto c5 = nodes_.Coords(ci.X() + 1, ci.Y()    , ci.Z() + 1);
        const auto c6 = nodes_.Coords(ci.X()    , ci.Y() + 1, ci.Z() + 1);
        const auto c7 = nodes_.Coords(ci.X() + 1, ci.Y() + 1, ci.Z() + 1);
        // residuals at coarse cell nodes
        const auto r0 = coarseNodes.residual_(ci.X()    , ci.Y()    , ci.Z());
        const auto r1 = coarseNodes.residual_(ci.X() + 1, ci.Y()    , ci.Z());
        const auto r2 = coarseNodes.residual_(ci.X()    , ci.Y() + 1, ci.Z());
        const auto r3 = coarseNodes.residual_(ci.X() + 1, ci.Y() + 1, ci.Z());
        const auto r4 = coarseNodes.residual_(ci.X()    , ci.Y()    , ci.Z() + 1);
        const auto r5 = coarseNodes.residual_(ci.X() + 1, ci.Y()    , ci.Z() + 1);
        const auto r6 = coarseNodes.residual_(ci.X()    , ci.Y() + 1, ci.Z() + 1);
        const auto r7 = coarseNodes.residual_(ci.X() + 1, ci.Y() + 1, ci.Z() + 1);

        // linear interpolation function
        auto linearInterp = [&fc](const auto &x0, const auto &x1, 
                                  const auto &d0, const auto &d1, auto &coord) {
          const auto dir = (x1 - x0).Normalize();
          const auto dist = x0.Distance(x1);
          const auto coeff = (fc - x0).DotProd(dir) / dist;
          coord = (1.0 - coeff) * x0 + coeff * x1;
          return (1.0 - coeff) * d0 + coeff * d1;
        };

        // 4 linear interpolations to convert to 2D
        vector3d<double> c04, c15, c26, c37;
        const auto r04 = linearInterp(c0, c4, r0, r4, c04);
        const auto r15 = linearInterp(c1, c5, r1, r5, c15);
        const auto r26 = linearInterp(c2, c6, r2, r6, c26);
        const auto r37 = linearInterp(c3, c7, r3, r7, c37);

        // 2 linear interpolations to convert to 1D
        vector3d<double> c0415, c2637;
        const auto r0415 = linearInterp(c04, c15, r04, r15, c0415);
        const auto r2637 = linearInterp(c26, c37, r26, r37, c2637);

        // 1 linear interpolation to complete trilinear interpolation
        vector3d<double> coord;
        auto resid = linearInterp(c0415, c2637, r0415, r2637, coord);
